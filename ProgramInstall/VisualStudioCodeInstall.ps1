<#
Download und Installation der aktuellste Version von Visual Studio Code

Idee von: https://github.com/silentinstalls/PowerShell/blob/main/VisualStudioCode.ps1
...
try {
    ...
    Write-Host "`nNeueste Visual Studio Code Version: " -NoNewline -ForegroundColor Yellow
    ...
    $installVSC = Show-ModernMessageBox -Title 'Visual Studio Code Installation' -Message "Der Download der aktuellen Visual Studio Code Version ist beendet. Soll die Version '$latestVersion' installiert werden? " -Icon Question YesNo
    if ($installVSC -eq 'Yes') {
        Start-Process -FilePath "$env:TEMP\VisualStudioCode-latest.exe" -ArgumentList '/VERYSILENT /NORESTART' -Wait
        Write-Host "`n✅ Visual Studio Code Installation abgeschlossen.`n" -ForegroundColor Green
    } else {
        Show-ModernMessageBox -Title 'Visual Studio Code Installation' -Message "Die Installation wurde vom Benutzer nicht zugelassen! Skript wird beendet" -Icon Information OK
        exit 0
    }

    # Bereinigen.
    $cleanVSC = Show-ModernMessageBox -Title 'Visual Studio Code Installation' -Message 'Soll die Installionsdatei gelöscht werden?' -Icon Question YesNo
    if ($cleanVSC -eq 'Yes') {
        Remove-Item -Path "$env:TEMP\VisualStudioCode-latest.exe" -Force -ErrorAction SilentlyContinue
        Write-Host "`n✅ Visual Studio Code Installationsdatei gelöscht.`n" -ForegroundColor Green
    } else {
        Show-ModernMessageBox -Title 'Visual Studio Code Installation' -Message "Die Installationsdatei 'VisualStudioCode-latest.exe' finden Sie zur weiteren Verwendungin Ihrem Temp-Verzeichnis unter '$env:TEMP'" -Icon Information OK
        exit 0
    }
} catch {
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCC0OCepNbL6zfyD
# 4zb+hKVDBL9470guY2S0WoEwsk8XyaCCF30wggQ/MIICJ6ADAgECAhAT5FOc42Tt
...
# KwYBBAGCNwIBFTAvBgkqhkiG9w0BCQQxIgQg9SXdRUGjmV4WwbVQ8UVnmmpWdl/x
# 93Q+XP9NCUcWr2YwDQYJKoZIhvcNAQEBBQAEggEAfZ6BHkGOXZ2qRN3+4P/7mjCy
# 7J1WzPytATf1bBe8vN3i6GmzgH6L45BbDNbjFTUD8xy5X1ZG1tYcH7C4Wdh2eaWs
# VvVTzf235x5GEACW6mLfMXq7Fs2ZZcoiIaiKgr7JSJvVS7A9th74ZXBrhQvRjcq/
# ELdVIrDr+Ks/t3IXKZz48xaRzn4fcjQQYhdESoOV8AWbavoY5WYpSPjXk+mtNVcd
# Y/O4riCAZN13PF/x6XuJlR2OR3uCMqsBNslPLlc3ZckQVI7XAryvHFAuCAnn86l/
# GhiDqv2UuwxBe10Gaggd4RInwqTufO2m3MuE7oJjrvdHe0KkIDPDC+u+jUiVq6GC
...
# hvcNAQcBMBwGCSqGSIb3DQEJBTEPFw0yNTA3MjExNjQwMDlaMC8GCSqGSIb3DQEJ
# BDEiBCBAmjbweOf56n9eHXGXQw6IoiqYak7Dr0WBjLNFT8ArAzANBgkqhkiG9w0B
# AQEFAASCAgAT/jvBi0r2Z68P08B3LsANqSk+BG+qR+4IUS5GLSemd9T7lRK7BZBT
# 3sG8oNK4Sce/5hnvDgGL12dsUK3NIAXsLx3xK3/OenaH/VhOCsd/VpZI0wkk3+bx
# mYx6WUqAbos/klgBiyv+/jamRk9YO3UMrKQ39b05Fxf6Zfhl5N1RG54/gWSo45y4
# rIOFEx3uMcSGarPWHnHFwyK5GM48DC2BHFIVG5b0Es1rYEWUyUpkZjySDU2x7oZY
# veGiuLdIP4unoLMnX6B2DtPhL5L9FHZ+a7OH7o7mHqdyL9Kie8x9xlC/PL15P+dU
# 4hoTctCQZEbXQEW4gbHwTjG9P1jIWlZvRqQrdvWMzDZxRNxhBB3PjUrVKxTE2fST
# Pa8lyuQ4NEhpqGxqC+jR1OyFvG7mUPaaqsIDUkj8ovBgsoj4kcK8fsjT9hnNdwz7
# aezlHWXuXj/Y2D1M4DIz4TF+ABkIhtwflWs0JpXLhldf/g05woB/x7Sv6xvPe7Hq
# 5axvQw9nmyL2Iv6/KqeqQro/X3jUX5D+VZokzHSudz8sINfATDEqCIgyMbaNxv/u
# 6M+nSP7FI4JbAPtbtGXgNy8ZRj8PAlBm+VHVDlOvA6k1FTC7LoS6vH3V/jsDpANh
# LTcxkJ7SCVoYLtnH/5jLJ53UYBSgx4b1XWEJdY2DnOpt8rKEjZzalA==
