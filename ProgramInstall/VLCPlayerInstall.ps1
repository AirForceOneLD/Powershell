<#
Download und Installation der aktuellste Version von VLC Player

Idee von: https://github.com/silentinstalls/PowerShell/blob/main/VLC.ps1
...
try {
    ...
    Write-Host "`nNeueste VLC Player Version: " -NoNewline -ForegroundColor Yellow
    ...
    $installVLC = Show-ModernMessageBox -Title 'VLC Player Installation' -Message "Der Download der aktuellen VLC Player Version ist beendet. Soll die Version '$latestVersion' installiert werden? " -Icon Question YesNo
    if ($installVLC -eq 'Yes') {
        Start-Process -FilePath "$env:TEMP\VLC-latest.exe" -ArgumentList '/S' -Wait
        Write-Host "`n✅ VLC Player Installation abgeschlossen.`n" -ForegroundColor Green
    } else {
        Show-ModernMessageBox -Title 'VLC Player Installation' -Message 'Die Installation wurde vom Benutzer nicht zugelassen! Skript wird beendet' -Icon Information OK
        exit 0
    }

    # Bereinigen.
    $cleanVLC = Show-ModernMessageBox -Title 'VLC Player Installation' -Message 'Soll die Installionsdatei gelöscht werden?' -Icon Question YesNo
    if ($cleanVLC -eq 'Yes') {
        Remove-Item -Path "$env:TEMP\VLC-latest.exe" -Force -ErrorAction SilentlyContinue
        Write-Host "`n✅ VLC Player Installationsdatei gelöscht.`n" -ForegroundColor Green
    } else {
        Show-ModernMessageBox -Title 'VLC Player Installation' -Message "Die Installationsdatei 'VLC-latest.exe' finden Sie zur weiteren Verwendung in Ihrem Temp-Verzeichnis unter '$env:TEMP'" -Icon Information OK
        exit 0
    }
} catch {
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBvmPRKkkAVLw0l
# G+oa7tLWN+9ZBs+QsVpSNVbEJV5OQ6CCF30wggQ/MIICJ6ADAgECAhAT5FOc42Tt
...
# KwYBBAGCNwIBFTAvBgkqhkiG9w0BCQQxIgQgxM94qJuPaD2pCUWDI5vxe7kZLsvO
# MFxKMRhwK8jUlREwDQYJKoZIhvcNAQEBBQAEggEATILBPGJOJkBIyJo8mEZRs1iE
# ycgErNssC6Ek50EJLy9YXDYK4+LsH3DlMeVEC7TPeOiKZJHwQulEh0UMfPdYOuvh
# n7EBlgcL54xHbYk/KA4XWjHc51oGk1z8VE5GsRdrhNTCoxkNydA74yDZgPXlRMnZ
# F4fb26wmyIUXZbccnDMKkptjA6PA7iDdk6ha5p/gfNtC5wOQ1ASzorc1cvJwb9yg
# g0YB/KrWqTYWZskIuA94UnRkAGSl0VYtOONJ9dPKFwSMrauQ3MHbQIxNOfweaI3G
# SBzKxmoMD/bSbplzM898GQsEZUo6dVpB1GmynZeZKHgTH+5Ax1oNb3G2Yj3laaGC
...
# hvcNAQcBMBwGCSqGSIb3DQEJBTEPFw0yNTA3MjExNzM1MjRaMC8GCSqGSIb3DQEJ
# BDEiBCDPYDhp1PovHnPeCuVWbagDwbwm2yqULIhpbK7OUN+5YjANBgkqhkiG9w0B
# AQEFAASCAgCFZWpHAq7YZzvCQ+oD/i/1+zI8tgV+yg2W+6JU2/k+/pxWm9s7avT1
# nGtQ6ccdxmbUvOCKX/tjkPwdLtpvZFehXLXK1Mg22sg/JuNUfO9ZUNhiB6eZImtg
# +p+/WeKnyMe21FyPWkq89c3Tex/HBrJrC7aHLBgKl1FIr//JmNc73BOTRjh8CAF8
# Iwqocv2zX0MAzfYLoeyPPwthmEI8lPabMbgZnN20euw5l6QHz3+CnbRriSo4PtZa
# cQI10VhqZc9aGsP4j1AVcgwgWpcccEld/GU5A7ssYNb9+uix+E6OvM/Webz0H/y2
# BDyMNmW/Hvv/Vz2zibjSHSeIJ89ZP3nelETcaCYMNTTTGVa/f4FemYruvT3AgcYo
# /QfI2f7iCpjpN7m4vlA2ltJYVcH6H6tEghNCYpH5FhwxQs+Xf4qXoEsM5uGmgBV9
# AxOYD6RVwWPmDIuIcryiNjaes1Ex4rQAljpNuqipW1d3fPulbwsAWNeAVUNmuVSP
# Bw95Fa6OE2ip1msjxaq5z8YT+ES4X9544s0W+7AbHbZ23/RZGUZiI3xHbUzjZ9Z7
# 52+uE3TkiOtCH0AF/NNVedbGK/W5G6aHqDr5PEt2LlakXSnbRZxYs7bXTZxBsc8+
# rdEmjBxzS0IrmHxYC/0jNMJQkKpt+yVDZo5qVfGwuhQ/qLJ6Xmylog==
