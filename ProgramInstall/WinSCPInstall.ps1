<#
Download und Installation der aktuellste Version von WinSCP

Idee von: https://github.com/silentinstalls/PowerShell/blob/main/WinSCP.ps1
...
try {
    ...
    Write-Host "`nNeueste WinSCP Version: " -NoNewline -ForegroundColor Yellow
    ...
    $installACP = Show-ModernMessageBox -Title 'WinSCP Installation' -Message "Der Download der aktuellen WinSCP Version ist beendet. Soll die Version '$latestVersion' installiert werden? " -Icon Question YesNo
    if ($installACP -eq 'Yes') {
        ...
        Write-Host "`n✅ WinSCP Installation abgeschlossen.`n" -ForegroundColor Green
    } else {
        Show-ModernMessageBox -Title 'WinSCP Installation' -Message 'Die Installation wurde vom Benutzer nicht zugelassen! Skript wird beendet' -Icon Information OK
        exit 0
    }

    # Bereinigen.
    $cleanACP = Show-ModernMessageBox -Title 'WinSCP Installation' -Message 'Soll die Installionsdatei gelöscht werden?' -Icon Question YesNo
    if ($cleanACP -eq 'Yes') {
        Remove-Item -Path "$env:TEMP\WinSCP-latest.exe" -Force -ErrorAction SilentlyContinue
        Write-Host "`n✅ WinSCP Installationsdatei und Installationsverzeichnis gelöscht.`n" -ForegroundColor Green
    } else {
        Show-ModernMessageBox -Title 'WinSCP Installation' -Message "Die Installationsdatei 'WinSCP-latest.exe' und Installationsverzeichnis finden Sie zur weiteren Verwendung in Ihrem Temp-Verzeichnis unter '$env:TEMP'" -Icon Information OK
        exit 0
    }
} catch {
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDzw3W4PjtUHLaK
# AO6cMVR6/KgzT6UDjC6YjuwXz9Y5uKCCF30wggQ/MIICJ6ADAgECAhAT5FOc42Tt
...
# KwYBBAGCNwIBFTAvBgkqhkiG9w0BCQQxIgQgpCn6urnL+X4zn6L4lx6PKyG/F+aw
# Mhias5xnhvjGAgQwDQYJKoZIhvcNAQEBBQAEggEAe0g2frZfybu8xCpfuLEY8T55
# nPYO81DjdzMABPI/D/l8IBWYxcrcQVfsbFibLQNjbrj6ReQhsoc6gv38csr0W5du
# KP/IFyLi3rK/RtwxDnPMjzSREJslG3+1N6IqIDjYTWiVxxNxKyLvc5mH1jaonl3b
# nOHE1KwGKTe1lNxauFx2dkb/5MhfGR4v19pN9Lwfvjo0NDnIrnTr2QclPpNCvf2N
# a5sI8w98+ThstQ2BGKx8W1Z+S6YBLNdlFESWwQGFB3Mn8GcKBU1tNz9RiAkZC2xB
# 1wNeZXjHd3T99W2p1fKr9XSlfgZPruqr7FfJhTy8Vm9WsX1O7bsZ1bnBjMnEK6GC
...
# hvcNAQcBMBwGCSqGSIb3DQEJBTEPFw0yNTA3MjIxMTUyMDFaMC8GCSqGSIb3DQEJ
# BDEiBCCe9xws/MJS/+Bj/1cby8mNiQTwv/Mdea7180fX/iQ8kDANBgkqhkiG9w0B
# AQEFAASCAgCZ7T2/hxdW5CJXKdRYFFrIbkhV9bnI6hRyDKtxLasLHvm9Uvn/NSu1
# kDI0qXcAfZVMszjda7QlfWjYQSccsg7Z2HZP+/W6SCydSVhVsCrPlyIxQXm0q7mn
# KS/jTwY09dw9ph3Xni9erfPvhZx3HVtRvZv9BdK30csHrZRrDAR0JY8h+LAHvetk
# cjTRK6reBbZRfY7F9eJ+MklkdW8Tzsr84MtCycMLUIHW5ANo4Dreqw+BBip7HFga
# 76ajZMNI9v0PL/zHq2MuigkGAPkKerUWg1ytQ4OZ4fjHTmtwKi6CZY/uWkcpK07/
# s298ivk4HCAKIOAXEWPChgjt10R0RsXayiQ37t6j0dYU7s5PQ9HP47S4Ul4fU7er
# pu68aodMR/yIdOBRRjSvkHr+eSuMVmkoZ896OYgmNw5guvaK2EONF+fUj/5Qj5xk
# roRHiruTIXyfX8GP/Y6UMrqdIV/d7biL4kFozwSOjflXXtZhIMckwfF5sAhEnLIC
# /afbvcmr3z9/ia0wqCVYSbDvz6EebWlEhtk+nphVYJNR2gJUGQky0B8Xt1/ECwQs
# N5ekTgnfgD14jRk9meue4M3G7LOkzNLgieNNpFo1pka9RI16XKNLUVFr2jxcsCt9
# IFP970xoO/wpyMBc7HP2ilfwlj/snMrlapqqrtp5gbCJgzM8zr42ag==
