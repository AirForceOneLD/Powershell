<#
Download und Installation der aktuellste Version von Adobe Acrobat Pro

Idee von: https://github.com/silentinstalls/PowerShell/blob/main/AcrobatPro.ps1
...
try {
    ...
    Write-Host "`nNeueste Adobe Acrobat Pro Version: " -NoNewline -ForegroundColor Yellow
    ...
    $installACP = Show-ModernMessageBox -Title 'Adobe Acrobat Pro Installation' -Message "Der Download der aktuellen Adobe Acrobat Pro Version ist beendet. Soll die Version '$latestVersion' installiert werden? " -Icon Question YesNo
    if ($installACP -eq 'Yes') {
        Start-Process -FilePath "$env:TEMP\Acrobat.Pro\Adobe Acrobat\setup.exe" -ArgumentList '/sAll /rs /msi' -Wait
        Write-Host "`n✅ Adobe Acrobat Pro Installation abgeschlossen.`n" -ForegroundColor Green
    } else {
        Show-ModernMessageBox -Title 'Adobe Acrobat Pro Installation' -Message 'Die Installation wurde vom Benutzer nicht zugelassen! Skript wird beendet' -Icon Information OK
        exit 0
    }

    # Bereinigen.
    $cleanACP = Show-ModernMessageBox -Title 'Adobe Acrobat Pro Installation' -Message 'Soll die Installionsdatei gelöscht werden?' -Icon Question YesNo
    if ($cleanACP -eq 'Yes') {
        ...
        Write-Host "`n✅ Adobe Acrobat Pro Installationsdatei und Installationsverzeichnis gelöscht.`n" -ForegroundColor Green
    } else {
        Show-ModernMessageBox -Title 'Adobe Acrobat Pro Installation' -Message "Die Installationsdatei 'Acrobat.Pro-latest.zip' und Installationsverzeichnis finden Sie zur weiteren Verwendung in Ihrem Temp-Verzeichnis unter '$env:TEMP'" -Icon Information OK
        exit 0
    }
} catch {
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCD5HTMtoaR/q+cC
# vmxFLI1gmZeDRw4sKjDsbYuTOH2J7qCCF30wggQ/MIICJ6ADAgECAhAT5FOc42Tt
...
# KwYBBAGCNwIBFTAvBgkqhkiG9w0BCQQxIgQg3xay+j7BXTwPrYVN0jyRhcnX1tQ7
# StxGAXoWL5NpqkowDQYJKoZIhvcNAQEBBQAEggEAkIlwjOWv3EjVhv+H+De772Iv
# hCikevda5aVy5Od3IT0a8fwTYa3Jx/w+sSzXWcpeSnkiEQSK/VUuxML6RfmR9GEk
# CU6fKrXli36HfZ+atOWBq8akcW7FhqzFHHhHwhgzGwmmT1WB4NV09ST3+0YyTYUe
# F98LDHOgEOapOkq4UCqN9UR1Oxbbv4KyYvHj91vP7UG3WoLQ6/ZfnZ7+x0GVw94H
# j4ugZDanlx4QUu/oKgcuxvNcfpACn251Oc56kTiozIemZiGcySdAmZ+MWUOT74SH
# IaAvnNsGPsnmXFpkh98WHxgiP+le87jaKLLYGT6vp9i5K9WnC5lX7yCJCChQoaGC
...
# hvcNAQcBMBwGCSqGSIb3DQEJBTEPFw0yNTA3MjIwODUzMzdaMC8GCSqGSIb3DQEJ
# BDEiBCC1mc3AvpoHdLcx8FJjQst2Z6ORZEEEvB/JApqjC3iaMTANBgkqhkiG9w0B
# AQEFAASCAgCYQhAYztx0ID73FxflEwBB38LOcqXCK5drgddMCqA7wTZ2xtfun/k+
# hQzKhgv7nJwhH2ArFFUrOHCC20AS6Y9aLMAyixEvkPu7zXBCjX08FUPjIwSHH942
# //lb8hwEp9o/NQTW5lmxQarnhQmB0WXE3d0MCmrycSFJzPCQLCb5/A7QKkGu5npu
# KTUEap9iHuv4Ir0rDXfQ/k8zywdkJQK7fYAyKJeikLtMd1vpdgBYl/Ec45wagW3r
# 3uv6lTdWrMvKKmNVp2SNfigS2APQvP04I5Mdb+6RdRufIa22G6RoHIvn1R3rJkWV
# T2yBoKEkbp0jcATFqJfbzrc0XzuJfgHmv5BxNT4Zo9/1pndaKlBCOicuCTKX5iBm
# 2i4J76c0dlGw3xZ0kf2KoAZvOlbstP5Cf94Z7aLi9FyxrzZFrQ/XKfk021McU9ad
# erN3qqH0gEl6m4PK5sjm/wFjYBN8hoLKFQCoO7LweX4XcZjkB4BYXPrzjYGZSgGW
# FIMupcYyi/1oJfTKeZfp3M/KpR9oFiOhq7lKkWLUZfbXwyTGtzzco/wPN0QG+act
# o8AkejQPdzppFX6XcDE1orEgFj0Mv9Y12dOgIM0n/2EbkDmLH4U9yVAkXuz3l6jU
# 39m1rMH7gsYcMZKCP4vJhAO8GP541B9TBbuMz7hvCafe9DzawWl73g==
